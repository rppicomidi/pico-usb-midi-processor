...
namespace rppicomidi
{
    ...
    public:
        ...
        bool set_message_type(size_t idx) { return message_type.set(idx); }
        void get_message_type(std::string &typestr) { message_type.get(typestr); }
        bool set_display_format(size_t idx) { return display_format.set(idx); }
        ...
        {
            auto me = reinterpret_cast<Midi_processor_chan_mes_remap *>(context_);
            return me->bimap.incr(bimap_idx_, element_idx_, delta_);
        }
